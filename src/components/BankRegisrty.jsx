...
const RegistryBank = () => {
  ...
  const showTextFunction = () => {
    ...
    // Verifica que validationResponse sea un array o string con la propiedad .length
    if (
      (typeof validationResponse === 'string' || Array.isArray(validationResponse)) &&
      validationResponse.length >= 0 || 
      (inputPassValue !== '' && inputNameValue !== '' && inputDNIValue !== '')
    ) {
