...
class CalendarioPrincipal extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      calStatus: "", // Nuevo estado para guardar el status
      calMonth: {}   // Nuevo estado para guardar CalMonth
    };
    this.localizer = dayjsLocalizer(dayjs);
  }
  ...
  async componentDidMount() {
    ...
    try {
      ...
      if (response.status === 200) {
        const { status, message, calories, CalMonth } = response.data;
        this.setState({
          cal: calories[0]?.value || 0,
          events: this.convertCaloriesToEvents(calories),
          calStatus: status,
          calMonth: CalMonth
        });
      } else {
  ...
  convertCaloriesToEvents = (calories) => {
    return calories.map(cal => ({
      start: dayjs(cal.date).toDate(),
      end: dayjs(cal.date).add(30, 'minute').toDate(),
      title: `Calorías: ${cal.value}`
    }));
  };
  ...
  saveCalories = async () => {
    const { calStatus, calMonth, cal } = this.state;
    const userEmail = sessionStorage.getItem('userEmail');
    const today = dayjs().startOf('day');
    ...
    switch (calStatus) {
      case "success":
        payload = {
          ...
          CalMonth: calMonth // Usa el mes y el día actual sin cambios
        };
        break;
      ...
      default:
        console.error("Estado desconocido:", calStatus);
        return; // No hacer nada si el estado no es reconocido
    }
